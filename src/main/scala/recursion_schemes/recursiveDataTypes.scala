...
object recursiveDataTypes {
  ...
  def multiplyFold: List => Int = foldList[Int](1, _ * _)

  def lengthFold: List => Int = foldList[Int](0, (_, len) => len + 1)
  ...
  def multiplyAlgebra: FAlgebra[ListF, Int] = {
    case NilF() => 1
    case ConsF(h, t) => h * t
  }
  ...
  def multiplyList: List => Int = cata(multiplyAlgebra, out)

  def range: Int => List = // ana
  ...
  def rangeF: Int => List = ana(rangeCoalgebra, inF)

  def factorial: Int => Int = n => // hylomorphism
...
object Runner2 extends App {
  ...
  println(
    s"unfold and fold to calculate factorial using hylo ${factorialF(20)}"
  )
