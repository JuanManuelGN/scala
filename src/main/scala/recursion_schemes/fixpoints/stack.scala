...
import recursion_schemes.fixpoints.stack.Stack.{done, more}
...
object stack {
  ...
  val stackFactorialAlgebra: Stack[Int] => Int = {
    case Done(result)    => result
    case More(acc, next) => acc * next
  }
}

import recursion_schemes.fixpoints.stack.{ana, cata, stackFactorialAlgebra, stackCoalgebra}
object Runner extends App {

  /** def ana[F[_] : Functor, A](coAlgebra: A => F[A]): A => Fix[F] = a => Fix(coAlgebra(a) map
    * ana(coAlgebra)) implicit val stackFunctor: Functor[Stack] = new Functor[Stack] { override def
    * map[A, B](sa: Stack[A])(f: A => B): Stack[B] = sa match { case Done(result) => Done(result)
    * case More(a, next) => More(f(a), next) } ana(stackCoalgebra).apply(5) =
    * Fix(More(Fix(More(Fix(More(Fix(More(Fix(More(Fix(Done(1)),1)),2)),3)),4)),5)) Step by Step
    ...
    * Fix(more(4, 5) map ana(stackCoalgebra)) = { more(4, 5) = More(4, 5) } Fix(More(4, 5) map
    * ana(stackCoalgebra)) = { map ana(stackCoalgebra) = More(ana(stackCoalgebra)(a), next) | a = 4
    * \| next = 5} Fix(More(ana(stackCoalgebra)(4), 5)) // aquí se produce la llamada recursiva a
    * ana(... ...
    */
  println(
  ...
  println(
    (ana(stackCoalgebra) andThen cata(stackFactorialAlgebra))(5)
  )
}
